...
class LightningDiTPipeline(DiffusionPipeline):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, torch_dtype=None, **kwargs):
        root = Path(pretrained_model_name_or_path)
        from ...models.transformers import LightningDiTTransformer2DModel
        from ...schedulers import LightningDiTFlowMatchScheduler

        transformer = LightningDiTTransformer2DModel.from_pretrained(
            root / "transformer", torch_dtype=torch_dtype
        )
        scheduler = LightningDiTFlowMatchScheduler.from_pretrained(root / "scheduler")

        vae = None
        vae_dir = root / "vae"
        vae_hint = root / "vae_pretrained_model_name_or_path.txt"
        if vae_dir.is_dir():
            from diffusers import AutoencoderKL

            vae = AutoencoderKL.from_pretrained(vae_dir, torch_dtype=torch_dtype)
        elif vae_hint.is_file():
            from diffusers import AutoencoderKL

            vae_id = vae_hint.read_text(encoding="utf-8").strip()
